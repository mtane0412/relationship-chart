...
/**
 * RelationshipFormのプロパティ
 */
type RelationshipFormProps = {
  /** 接続元の人物情報 */
  sourcePerson: PersonInfo;
  /** 接続先の人物情報 */
  ...
  /** 登録/更新ボタンクリック時のコールバック */
  onSubmit: (
    type: RelationshipType,
    sourceToTargetLabel: string,
    targetToSourceLabel: string | null
  ) => void;
  /** キャンセルボタンクリック時のコールバック（省略可能） */
  onCancel?: () => void;
  /** オートフォーカスを有効にするか（省略時はtrue） */
  autoFocus?: boolean;
};
 ...
 * ラベル入力の方向コンテキスト表示に使用
 */
function PersonMiniIcon({ person, testId }: { person: PersonInfo; testId: string }) {
...
export function RelationshipForm({
  ...
  }, [defaultType, initialRelationship, autoFocus]);
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <form onSubmit={handleSubmit} className="flex-1 flex flex-col">
        ...
        {/* ボタン */}
        <div className={`flex gap-3 ${onCancel ? 'justify-end' : 'justify-stretch'} mt-auto`}>
          {onCancel && (
            <button
              type="button"
              onClick={onCancel}
          ...
          <button
            ...
            className={`px-4 py-2 text-sm font-medium text-white bg-blue-600 rounded-md hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 disabled:bg-gray-300 disabled:cursor-not-allowed transition-colors ${onCancel ? '' : 'flex-1'}`}
          >
            {isEditMode ? '更新' : '登録'}
          </button>
        </div>
